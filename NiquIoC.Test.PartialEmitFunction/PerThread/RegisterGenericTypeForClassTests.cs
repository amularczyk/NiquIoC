...
namespace NiquIoC.Test.PartialEmitFunction.PerThread
{
    ...
    public class RegisterGenericTypeForClassTests
    {
        ...
        public void RegisterSimpleGenericClass_Success()
        {
            ...
            {
                genericClass = c.Resolve<GenericClass<EmptyClass>>(ResolveKind.PartialEmitFunction);
            });
            thread.Start();
            thread.Join();
        ...
        public void RegisterGenericClassWithClassWithNestedClass_Success()
        {
            ...
            var thread = new Thread(container =>
            {
                genericClass = c.Resolve<GenericClass<SampleClass>>(ResolveKind.PartialEmitFunction);
            });
            thread.Start();
            thread.Join();
        ...
        public void RegisterGenericClassWithManyParameters_Success()
        {
            ...
            {
                genericClass =
                    c.Resolve<GenericClassWithManyParameters<EmptyClass, SampleClass>>(ResolveKind
                        .PartialEmitFunction);
            });
            thread.Start();
            thread.Join();
