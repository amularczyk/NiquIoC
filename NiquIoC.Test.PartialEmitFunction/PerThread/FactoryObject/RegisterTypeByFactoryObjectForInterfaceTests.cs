    ...
    public class RegisterTypeByFactoryObjectForInterfaceTests
    {
        ...
        public void FactoryObjectReturnNewObject_Success()
        {
            ...
            var thread = new Thread(() => {
                sampleClass1 = c.Resolve<ISampleClassWithInterfaceAsParameter>(ResolveKind.PartialEmitFunction);
                sampleClass2 = c.Resolve<ISampleClassWithInterfaceAsParameter>(ResolveKind.PartialEmitFunction);
            });
            thread.Start();
            thread.Join();
