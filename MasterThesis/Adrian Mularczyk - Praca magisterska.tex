...
Rys. \ref{fig:Solid_without} przedstawia przykładową klasę bez zastosowania zasady DIP.\\
\begin{figure}[H]
	\begin{center}
	...
	\end{center}
\end{figure}
Rys. \ref{fig:Solid_with} przedstawia tę samą klasę po zastosowaniu zasady DIP.\\
\begin{figure}[H]
	\begin{center}
...
Załóżmy, że mamy dwie klasy: \textbf{A} i \textbf{B}, zdefiniowane jak na Rys. \ref{fig:Example_Container}. \\ \\
\begin{figure}[H]
	\begin{center}
...
\end{figure}
\clearpage
Rejestracja tych klas w kontenerze wygląda następująco: \\
\\
\texttt{container.Register<A>();\\
container.Register<B>();}\\
\\
Obiekty tych klas (przy pomocy kontenera) można uzyskać w następujący sposób: \\
\\
\texttt{container.Resolve<B>();\\
container.Resolve<A>();}\\
\\
Stworzenie obiektu klasy \textbf{B} przy pomocy operacji \textbf{Resolve} jest proste, ponieważ ta klasa nie ma żadnych zależności. Dla obiektu klasy \textbf{A} ta operacja jest już bardziej skomplikowana, ponieważ wymaga przejrzenia struktury kodu i odkrycia, że do wygenerowania obiektu klasy \textbf{A} wymagany jest obiekty klasy \textbf{B}.\\
\\
Operacja \textbf{Resolve} może się jeszcze bardziej skomplikować, gdy zostanie dodana polityka czasu życia. Jeśli klasa \textbf{B} zostanie zarejestrowana jako singleton:\\
\\
...
\begin{figure}[H]
	...
	\end{center}
\end{figure}
Możliwość rejestracji implementacji dla wskazanej abstrakcji jest kluczem do spełnienia postulatów reguły DIP przez kontener. Kontener otrzymuje wtedy pełną wiedzę i odpowiedzialność za koordynowanie rozwiązywania zależności. Rekonfiguracja zależności wymaga tylko i wyłącznie zmiany konfiguracji, którą zna tylko kontener.\\
\\
Dzięki takiemu podejściu programowanie rozległej aplikacji staje się "projektowaniem jej architektury" - określaniem miękkich zależności między klasami (przez abstrakcje) i komponowaniem ostatecznego kształtu aplikacji (wskazywaniem jak implementowane są abstrakcje) w miejscu konfiguracji kontenera.\\
\\
Warto dodać, że wiele kontenerów wspiera konfigurację deklaratywną (w postaci plików np. XML) co powoduje, że aplikację można przekonfigurować bez potrzeby jej kompilacji, wyłącznie przez zmianę konfiguracji (mapowania abstrakcji na implementacje) w plikach konfiguracyjnych. 
...
Na rynku jest wiele implementacji wstrzykiwania zależności. Poniżej przedstawiono kilka najbardziej popularnych (według ilości pobrań z NuGet) oraz kilka najszybszych (według rankingu na blogu Daniela Palme \cite{IoCBenchmark}).\\
\\
Dane pochodzą z dnia 21-02-2017. W nawiasie znajduje się wersja implementacji, która została użyta w testach (najnowsza na dany dzień).\\
\\
...
\texttt{  IL\_0000:  ldarg.1 }\\
// umieść 0 na szczycie stosu \\
\texttt{  IL\_0001:  ldc.i4.0 }\\
// jeśli pierwsza wartość na szycie stosu jest większa od drugiej, idź do operacji IL\_006 \\
\texttt{  IL\_0002:  blt.s      IL\_0006 }\\
// wczytaj pierwszy argument funkcji i umieść go na szczycie stosu \\
\texttt{  IL\_0004:  ldarg.1 }\\
// zwróć wartość ze szczytu stosu \\
\texttt{  IL\_0005:  ret }\\
// wczytaj pierwszy argument funkcji i umieść go na szczycie stosu \\
\texttt{  IL\_0006:  ldarg.1 }\\
...
\subsection{Porównanie z istniejącymi implementacjami}
Rozwiązanie przedstawione w tej pracy zostało zainspirowane artykułem przedstawiającym rozwiązanie \textbf{Hiro} \cite{Hiro}.\\
\\
Ideą \textbf{Hiro} było najpierw zbudowanie mapy wszystkich zależności, a następnie zapamiętaniu tej mapy i wykorzystaniu za każdym razem, gdy obiekt danego typu będzie tworzony. W tym rozwiązaniu nie użyto refleksji, a mapa zależności jest zapisana w dodatkowym assembly tworzonym dynamicznie.\\
\\
Niestety to rozwiązanie nie wyszło z fazy proof-of-concept, a aktualna wersja dostępna w repozytorium autora nie działała i od kilku lat nie jest aktualizowana, stąd brak porównania z nim.\\
\\
\\
Rozwiązaniem najbardziej podobnym do rozwiązania przedstawionego w tej pracy wydaje się rozwiązanie \textbf{DryIoc}. To rozwiązanie również wykorzystuje \textbf{Reflecion.Emit}, jednakże działa ono trochę inaczej. W tym rozwiązaniu najpierw tworzone jest drzewo (\textbf{Expression}) zależności dla każdego typu, a dopiero później na jego podstawie tworzony jest delegat z wykorzystaniem \textbf{Reflecion.Emit}. W związku z tym stworzenie pierwszego obiektu zajmuje dużo więcej czasu niż w przypadku \textbf{NiquIoCFull}, ale potem stworzenie każdego kolejnego obiektu zajmuje tego czasu trochę mniej. Niestety ten narzut dla pierwszego wywołania sprawia, że nawet przy tworzeniu wiele razy tego samego obiektu \textbf{DryIoc} osiąga większe czasy niż \textbf{NiquIoCFull}. Dodatkowo dla rejestracji typu \textbf{PerThread}, to rozwiązanie nie tworzy delegata z wykorzystaniem \textbf{Reflection.Emit}. Jest to powodem dużych wyników dla testów z wykorzystaniem tego typu rejestracji.\\
\\
\\
Jeśli chodzi o najwolniejsze rozwiązania, to one prawdopodobnie nie optymalizują kompozycji grafu zależności, tylko przechodzą go za każdym razem. Takie przechodzenie nie wydaje się konieczne, ponieważ tak długo jak kod nie jest rekompilowany, to struktura zależności nie ulega zmianie.
...
\bibitem{csharp} Ian Griffiths, Matthew Adams, Jesse Liberty, C\#. Programowanie, Wydanie VI, Helion, 2012
\bibitem{dependency_injection} Mark Seemann, Dependency Injection in .NET, Manning, 2012
\bibitem{emit} Serge Lidin, Expert .NET 2.0 IL Assembler, Apress, 2006
\bibitem{clean_code} Robert C. Martin, Czysty kod. Podręcznik dobrego programisty, Helion, 2014
\bibitem{SOLID} \url{https://en.wikipedia.org/wiki/SOLID_(object-oriented_design)}
\bibitem{IoCBenchmark} Daniel Palme, IoC Container Benchmark - Performance comparison\\
\url{http://www.palmmedia.de/Blog/2011/8/30/ioc-container-benchmark-performance-comparison}
\bibitem{Hiro} Philip Laureano, Introducing Hiro, the World's Fastest IOC Container, Part I: Design Diary\\
\url{https://www.codeproject.com/Articles/35345/Introducing-Hiro-the-Worlds-Fastest-IOC-Container-.aspx}
\end{thebibliography}
