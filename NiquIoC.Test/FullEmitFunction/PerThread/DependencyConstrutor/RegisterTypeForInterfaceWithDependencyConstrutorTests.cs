...
namespace NiquIoC.Test.FullEmitFunction.PerThread.DependencyConstrutor
{
    ...
    public class RegisterTypeForInterfaceWithDependencyConstrutorTests
    {
        ...
        public void RegisteredInterfaceAsClassWithInterfaceAsParameterAndWithConstructorWithAttributeDependencyConstrutor_Success()
        {
            ...
            var thread = new Thread(() => { sampleClass = c.Resolve<ISampleClassWithInterfaceAsParameter>(ResolveKind.FullEmitFunction); });
            thread.Start();
            thread.Join();
        ...
        public void RegisteredInterfaceAsClassWithNestedInterfaceAsParameterWithInterfaceAsParameterAndWithConstructorWithAttributeDependencyConstrutor_Success()
        {
            ...
            var thread = new Thread(() => { sampleClass = c.Resolve<ISampleClassISampleClassWithInterfaceAsParameter>(ResolveKind.FullEmitFunction); });
            thread.Start();
            thread.Join();
